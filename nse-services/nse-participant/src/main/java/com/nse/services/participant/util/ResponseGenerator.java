...
import com.nse.services.participant.model.ParticipantJsonResponse;

import com.nse.services.participant.model.ParticipantJson;
...
public final class ResponseGenerator {
    ...
    public ParticipantJsonResponse generateResponse(final Map<Date, Participant> request) {
        ParticipantJson resultPercentage = new ParticipantJson();
        ParticipantJsonResponse response = new ParticipantJsonResponse();
        response.setDate(request.keySet().stream().findAny().get());

        Participant participant = request.entrySet().stream().findFirst().get().getValue();
