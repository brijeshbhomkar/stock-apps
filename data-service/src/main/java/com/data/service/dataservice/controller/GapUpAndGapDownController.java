...
import com.data.service.dataservice.repository.GapDownRepository;
import com.data.service.dataservice.repository.GapUpRepository;
...
//	private KiteDataService kiteDataService;
	...
	@Autowired
	private GapUpRepository gapUpRepository;

	@Autowired
	private GapDownRepository gapDownRepository;
//
...
//		return new ResponseEntity<List<StockWatch>>(
//				stocks.stream().sorted(Comparator.comparing(StockWatch::getTrdVol).reversed()).collect(Collectors.toList()),
//				HttpStatus.OK);
//	}
//
...
//	}
	...
	@RequestMapping("/strategy/gapup")
	public ResponseEntity<?> gapup() {
		final List<GapDownStock> result = gapDownRepository.findAll();
		if (CollectionUtils.isEmpty(result)) {
			return ResponseEntity.ok(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<GapDownStock>>(result, HttpStatus.OK);
	}

	@RequestMapping("/strategy/gapdown")
	public ResponseEntity<?> gapdown() {
		final List<GapUpStock> result = gapUpRepository.findAll();
		if (CollectionUtils.isEmpty(result)) {
			return ResponseEntity.ok(HttpStatus.NO_CONTENT);
		}
		return new ResponseEntity<List<GapUpStock>>(result, HttpStatus.OK);
	}
